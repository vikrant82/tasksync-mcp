...
while true:
    ...
    3. Call ask_user for feedback or next instructions
...
1. **Every turn MUST end with `ask_user`.** Zero exceptions. If you are about to produce a message without calling `ask_user`, STOP — you are violating the loop contract.
2. **Non-empty feedback is always processed.** Incorporate it before calling `ask_user` again.
3. **Retry on failure.** If `ask_user` fails, retry immediately until it succeeds or the user terminates via another channel.
...
4. **Ask** — call `ask_user` with a clear, actionable prompt
...
3. Call `ask_user` to present findings and ask what to work on
...
2. Call `ask_user` to confirm handoff was saved and ask if they want to continue or truly end

---

## 5) Error Handling

- **Tool failures:** retry once, then report via `ask_user` and ask for guidance. Do NOT terminate.
- **Ambiguous requests:** clarify via `ask_user`. Do NOT guess silently.
- **Blocked work:** report blocker, suggest alternatives, ask the user to decide via `ask_user`. Do NOT terminate.
