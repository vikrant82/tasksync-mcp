...
- **`get_feedback` errors (MCP resets, connection closed, timeouts):** These are transient. Retry up to 3 times. If still failing, report the error inline and keep calling `get_feedback` on your next turn — it will reconnect. **Never stop the loop, never write a handoff, never treat this as session termination.**
