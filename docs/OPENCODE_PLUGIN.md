...
6. If images were attached, the `tool.execute.after` hook injects them as native `FilePart` attachments — the LLM sees images directly
7. Agent processes feedback and calls `get_feedback` again
...
When remote mode is enabled for a session, the plugin captures the agent's most recent response text and sends it to the server as context. The server forwards it to configured notification channels (currently Telegram).
...
**Setup:** See the [main README](../README.md#remote-mode-telegram) for Telegram bot configuration.

## Limitations

- **Image support**: When users attach images, the plugin injects them as native `FilePart` attachments on the tool result via the `tool.execute.after` hook. The LLM sees images directly without needing file-reading tools. This approach works with OpenCode's schema validation (PartBase fields are generated by the plugin). The MCP integration provides similar native `ImageContent` blocks.
